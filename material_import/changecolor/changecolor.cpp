...
namespace  CHANGE_COLOR {
	struct Float4
	{
		...
		Float4(float r, float g, float b)
			: r(r),
			g(g),
			b(b),
			a(1.f) {}

		Float4(float f) : r(f), g(f), b(f), a(f) {}
		...
		Float4 operator-(const Float4& rhs) {
			return Float4{ r - rhs.r, g - rhs.g, b - rhs.b, a - rhs.a };
		}
	...
	Float4 RGBToHSV(const Float4& rgb) {
	...
	Float4 HSVToRGB(const Float4& hsv) {
		...
		return Float4{ rgb.r, rgb.g, rgb.b, hsv.a };
	}
	...
	Result changeColor(unsigned char* data, int picWidth, int picHeight, int picChannel) {
		...
		auto toOpenCVFormatHSV = [](const Float4& hsv) -> Float4 {
			return Float4{ hsv.r * 180, hsv.g * 255, hsv.b * 255, hsv.a };
		};

		auto getColor = [&](int pixelIdx) -> Float4 {
			auto pixelStart = pixelIdx * picChannel;
			if (picChannel == 3) {
				return {
					float(data[pixelStart]) / 255.f,
					float(data[pixelStart + 1]) / 255.f,
					float(data[pixelStart + 2]) / 255.f,
					1
				};
			}
		...
		std::vector<float> sumV(threadCount);
		std::fill(sumV.begin(), sumV.end(), 0.f);
		std::vector<float> maxS(threadCount);
		std::fill(maxS.begin(), maxS.end(), 0.f);
		std::vector<float> maxV(threadCount);
		std::fill(maxV.begin(), maxV.end(), 0.f);
		std::vector<float> minS(threadCount);
		...
		auto calculateParams = [&](int threadIdx, int threadCnt) -> void {
			...
			for (int row = threadIdx; row < height; ++row) {
				for (int col = 0; col < width; col += threadCnt) { // some thread will have more loops than others
		...
		for (int row = 0; row < height; ++row) {
			for (int col = 0; col < width; ++col) { // some thread will have more loops than others
				...
				auto color = getColor(pixelIdx);
				auto hsv = RGBToHSV(color);
				hsv = Float4{ hsv.r * 180, hsv.g * 255, hsv.b * 255, hsv.a };
		...
		float maxValueS = *std::max_element(maxS.begin(), maxS.end());
		float maxValueV = *std::max_element(maxV.begin(), maxV.end());
		float minValueS = *std::min_element(minS.begin(), minS.end());
		float minValueV = *std::min_element(minV.begin(), minV.end());
		...
		float bottomS = abs(minValueS);
		float bottomV = abs(minValueV);
		float topS = 255.f - maxValueS;
		float topV = 255.f - maxValueV;

		// refine the input color
		float kS = (topS - bottomS) / 255.f;
		float kV = (topV - bottomV) / 255.f;

		return { bottomS, bottomV, meanS, meanV, kS, kV };
		...
		//		saturateZero ? 0 : float(tableS[(unsigned char)(cvt.g)]) / 255.f,
		//		float(tableV[(unsigned char)(cvt.b)]) / 255.f,
		//		hsv.a
		//	};
		//	return res;
		//};
