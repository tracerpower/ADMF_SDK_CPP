...
zip_helper::ZIPResult ZipArchiveEntry::ExtractFile(const std::string& destinationPath)
{
 ...
 */
    ...
    std::istream* dataStream = this->GetDecompressionStream();

    if (dataStream == nullptr)
    {
        return zip_helper::ZIPResult::WRONG_PASSWORD;
    }
...
zip_helper::ZIPResult ZipArchiveEntry::ExtractData(unsigned char* buff, size_t& len)
{
    ...
    {
        len = 0;
        return zip_helper::ZIPResult::BUFFER_SIZE_NOT_ENOUGH;
    }

    size_t size_ = std::min(len, totalSize);
/*
    if (!password.empty())
    {
        this->SetPassword(password);
...
*/
    ...
    dataStream->read(reinterpret_cast<char*>(buff), size_);
    auto readSize = static_cast<size_t>(dataStream->gcount());
    len = readSize;
    if (readSize != size_)
    {
